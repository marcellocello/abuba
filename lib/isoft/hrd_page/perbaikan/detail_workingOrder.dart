...
//            }
    ...
    Firestore.instance.collection('user').where('grade', isEqualTo: 'Manager').where('department', isEqualTo: 'HRD').snapshots().listen((getDataManagerHRD) {
      setState(() {
        managerHRD = getDataManagerHRD.documents[0].data['id'];
      });
    });

    Firestore.instance.collection('user').where('grade', isEqualTo: 'Assistant Manager').where('department', isEqualTo: 'HRD').snapshots().listen((getAsManagerHRD) {
      setState(() {
        asManagerHRD = getAsManagerHRD.documents[0].data['id'];
      });
    });
  }
