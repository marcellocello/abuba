...
class _WorkingOrderState extends State<WorkingOrder> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    Firestore.instance.collection('user').where('grade', isEqualTo: 'Manager').where('department', isEqualTo: 'HRD').snapshots().listen((data3) {
      setState(() {
        managerHRD = data3.documents[0].data['id'];
      });
    });

    Firestore.instance.collection('user').where('grade', isEqualTo: 'Assistant Manager').where('department', isEqualTo: 'HRD').snapshots().listen((data4) {
      setState(() {
